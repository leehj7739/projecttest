...
export default function Header() {
    ...
    return (
        <header className={`w-full theme-card sticky z-40 transition-all duration-200 ${isScrolled ? 'top-8' : 'top-0'}`}>
            <div className="max-w-7xl mx-auto flex items-center justify-between h-16 px-4">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <button
                        ...
                        className={`px-3 py-2 rounded font-semibold border border-[var(--color-border)] theme-card theme-text focus:outline-none flex items-center gap-2 min-w-[90px] ${isDevMode ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'}`}
                        aria-label="모드 전환"
                    >
                    ...
                    {isAuthenticated ? (
                        ...
                        <Link to="/signin" className="hidden md:inline-block px-4 py-2 rounded font-semibold theme-accent border border-[var(--color-accent)] hover:bg-[var(--color-bg)] transition">
                            Sign in
                        </Link>
                    )}
                </div>
